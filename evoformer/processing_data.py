    ...
    def __init__(self, data: pd.DataFrame, time_granularity: str, dataset_name: str):
        ...
        if time_granularity != '' :
            ...
            if 'weight' not in data.columns:
                data['weight'] = 1
        # 这行代码使用了Pandas库的条件索引，对DataFrame进行筛选，选择满足指定条件的行
        if dataset_name == 'facebook':
            ...
            data = data[
                ((data['year'] == 2006) & (data['month'] <=8) ) | ((data['year'] == 2005) & (data['month'] >= 9))]
        elif dataset_name == 'Tw-WorldCup':
            data = data[(data['year'] == 2014) & (data['month'] == 6) & (data['day'] == 12) ]
        self.data = data
...
if __name__ == '__main__':
    ...
    # graphs = temporal_graph.get_temporal_graphs(min_degree=5,mode='static')
    # with open('../data/game_of_thrones/gameofthrones_2017_graphs_static.gpickle','rb') as f1:
    #     staticgraphs = pickle.load(f1)
    # with open('../data/game_of_thrones/gameofthrones_2017_graphs_dynamic.pkl','rb') as f2:
    #     dynamicgraphs = pickle.load(f2)



    with open('../data/game_of_thrones/gameofthrones_2017_graphs_dynamic.pkl','rb') as f:
        graphs = pickle.load(f)
    # results = {key : value for key,value in zip(dynamicgraphs.keys(),staticgraphs)}
    # with open('../data/game_of_thrones/game_static.pkl','wb') as f:
