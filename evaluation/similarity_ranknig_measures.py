...
from graphert.temporal_embeddings import get_temporal_embeddings
# from graphert.train_model import BertForMlmTemporalClassification, BertForTemporalClassification,TemporalAttentionLayer
...
def spearman_ranking(predicted, gt):
    ...
    for t, graph in predicted.iterrows():
        ...
        real_rank = np.argsort(gt.loc[t].values)
...
def eval_similarity(graph_embs: np.array, times: list, similarity_matrix_gt: pd.DataFrame):
    ...
    res_df = res_df.append(pd.DataFrame({'metric':["sp"],
                                         'value':[spearman_ranking(predicted,similarity_matrix_gt)]}))
    res_df = res_df.append(pd.DataFrame({'metric': ["kr"],
                                         'value': [kendalltau_ranking(predicted, similarity_matrix_gt)]}))
    return res_df
...
if __name__ == "__main__":
    ...
    # graphs = temporal_graph.get_temporal_graphs(min_degree=5)
    # with open('../data/enron/dynamic_graphs.pkl','rb') as f:
    ...
    model_path = f'../data/{dataset_name}/models/ep10_mask_walk32_model'
    # get temporal embeddings by the last layer
    temporal_embeddings = get_temporal_embeddings(model_path)
    # print(type(temporal_embeddings))

    # times = list(graphs.keys())
    # print(times)
    # x = eval_similarity(graph_embs=temporal_embeddings, times=list(graphs.keys()),
