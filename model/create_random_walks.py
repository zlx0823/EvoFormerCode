...
from graphert import backtrack
...
def create_random_walks(graphs: dict, ps: list, qs: list, walk_lengths: list, num_walks_list: list,
    ...
    for walk_len in walk_lengths:
        for num_walks in num_walks_list:
            ...
            # file_path = f'../data/{dataset_name}/paths_walk_len_{walk_len}_num_walks_{num_walks}.csv'
            file_path = f'../data/{dataset_name}/parameters.csv'
            data_df_list = []
            nodes = set()
            p_q_pairs = list(itertools.product(ps, qs))
            for i, (time, graph) in enumerate(tqdm(list(graphs.items()), desc="Processing graphs")):
                for (p, q) in tqdm(p_q_pairs, total=len(p_q_pairs),disable=True):
                    if filter_cc_nodes:
                        graph.remove_nodes_from(
                            [node for node in graph if
                             node not in filter_cc_nodes])  # remove nodes not in the biggest cc
                    graph = graph.to_undirected()
                    nodes = nodes.union(graph.nodes())
                    n2v_model = Node2Vec(graph, num_walks=num_walks, walk_length=walk_len, p=p, q=q, workers=5,
                                         quiet=True)
...
def create_random_walks_back(graphs: dict, ps: list, qs: list, walk_lengths: list, num_walks_list: list,
                             dataset_name: str, filter_cc_nodes: list = None):
    for walk_len in walk_lengths:
        for num_walks in num_walks_list:
            print(f"walk_len={walk_len}, num_walks={num_walks}")
            file_path = f'../data/{dataset_name}/static_paths_walk_back_len_{walk_len}_num_walks_{num_walks}.csv'
            p_q_pairs = list(itertools.product(ps, qs))
            data_df_list = []
            for (p, q) in tqdm(p_q_pairs, total=len(p_q_pairs)):
                d_graph_list = backtrack.precompute_probabilities(graphs, p, q, 'weight', filter_cc_nodes)
                data_df_list = backtrack.genera_walks(num_walks, 5, graphs, d_graph_list, walk_len, p, q, data_df_list)
