...
#     def __init__(self,vocab = 0,d_model = 384,dropout = 0.2,head = 1):
#         super(Encoder_layer,self).__init__()
#         self.MHA = MultiHeadAtten(head=head,embedding_dim=d_model,dropout=dropout)
#         self.SLC1 = SublayerConnection(d_model,dropout=dropout)
#         self.SLC2 = SublayerConnection(d_model,dropout=dropout)
#         self.FF =  FeedForward(d_model,1024,dropout=dropout)
#         self.d_model = d_model
#
#     def forward(self,x):
#         MHAR = lambda x: self.MHA(x, x, x)
#         SLC1R = self.SLC1(x, MHAR)
#         SLC2R = self.SLC2(SLC1R, self.FF)
#         SLC2R = SLC2R.view(-1, self.d_model)
#         return SLC2R

# class Transormer_encoder(nn.Module):
#     def __init__(self,layer,N,d_model = 384):
#         super(Transormer_encoder,self).__init__()
#         self.vocab = vocab
#         self.d_model = d_model
#         self.TE = TextEmbedding(vocab,d_model)
#         self.PE = PositionalEnconding(d_model,dropout,max_len=61)
#         self.layers
...
class Transormer_encoder(nn.Module):
    def __init__(self,vocab = 0,d_model = 384,dropout = 0.2,head = 1):
        ...
        self.TE = TextEmbedding(vocab,d_model)
        self.PE = PositionalEnconding(d_model,dropout)
        self.MHA = MultiHeadAtten(head=head,embedding_dim=d_model,dropout=dropout)
        self.SLC1 = SublayerConnection(d_model,dropout=dropout)
        self.SLC2 = SublayerConnection(d_model,dropout=dropout)
        self.FF =  FeedForward(d_model,1024,dropout=dropout)
...
if __name__ == '__main__':
    ...
    d_model = 32
    dropout = 0.2
    inputs = torch.arange(60).view(6, 10)
    # print(inputs)
    # 实例化文本嵌入层对象
    ...
    # LNR = LN(MHAR)
    # print(LNR.shape)
    # mask = torch.tensor([1 if i < 5 else 0 for i in range(61)])
    # print(mask)
